...
public final class TrainingAdminController: RouteCollection {
    ...
    public func boot(routes: RoutesBuilder) throws {
        let trainingRoutes = setupRoutes(routes)
        registerGetRoutes(trainingRoutes)
        registerPostRoutes(trainingRoutes)
        registerPutRoutes(trainingRoutes)
        registerDeleteRoutes(trainingRoutes)
    }
}
...
extension TrainingAdminController {
    ...
    fileprivate func registerGetRoutes(_ routes: RoutesBuilder) {
        ...
        routes.grouped(uuidMiddleware)
            .get("trainer", ":trainer-id", use: getTrainingByTrainerId)
            .openAPI(
                tags: .init(name: "Admin - Training"),
        ...
        routes.grouped(uuidMiddleware)
            .get("room", ":room-id", use: getTrainingByRoomId)
            .openAPI(
                tags: .init(name: "Admin - Training"),
    ...
    fileprivate func registerDeleteRoutes(_ routes: RoutesBuilder) {
        routes.grouped(uuidMiddleware)
            .delete(":id", use: deleteTrainingById)
            .openAPI(
                tags: .init(name: "Admin - Training"),
...
extension TrainingAdminController {
    @Sendable
    func getAllTrainings(req: Request) async throws -> Response {
    ...
    func createTraining(req: Request) async throws -> Response {
        guard
            let training = try await trainingService.create(
                try req.content.decode(TrainingCreateDTO.self)
            )
        else {
            throw TrainingError.invalidCreation
        }
        return try await TrainingDTO(from: training).encodeResponse(
            status: .ok,
            for: req
        )
    }
    ...
    @Sendable
    func updateTrainingById(req: Request) async throws -> Response {
        guard
            let training = try await trainingService.update(
                id: try req.parameters.require("id", as: UUID.self),
                with: try req.content.decode(TrainingUpdateDTO.self)
            )
        else {
    ...
    @Sendable
    func deleteTrainingById(req: Request) async throws -> HTTPStatus {
    ...
    @Sendable
    func getTrainingById(req: Request) async throws -> Response {
        guard
    ...
    @Sendable
    func getTrainingByTrainerId(req: Request) async throws -> Response {
        return try await trainingService.find(
            trainerId: try req.parameters.require("trainer-id", as: UUID.self)
        ).map {
            TrainingDTO(from: $0)
        }.encodeResponse(status: .ok, for: req)
    }
    ...
    @Sendable
    func getTrainingByRoomId(req: Request) async throws -> Response {
        return try await trainingService.find(
            trainingRoomId: try req.parameters.require("room-id", as: UUID.self)
        ).map {
            TrainingDTO(from: $0)
        }.encodeResponse(status: .ok, for: req)
    }
