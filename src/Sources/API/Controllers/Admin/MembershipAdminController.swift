...
public final class MembershipAdminController: RouteCollection {
    ...
    public func boot(routes: RoutesBuilder) throws {
        let membershipRoutes = setupRoutes(routes)
        registerGetRoutes(membershipRoutes)
        registerDeleteRoutes(membershipRoutes)
        registerPostRoutes(membershipRoutes)
        registerPutRoutes(membershipRoutes)
    }
}
...
extension MembershipAdminController {
    fileprivate func setupRoutes(_ routes: RoutesBuilder) -> RoutesBuilder {
        routes.grouped("admin", "memberships")
    ...
    fileprivate func registerGetRoutes(_ routes: RoutesBuilder) {
        routes.get("all", use: getAll).openAPI(
            ...
            description:
                "Возвращает список всех абонементов. Требует прав администратора.",
        ...
        routes.grouped(uuidMiddleware).get(":id", use: getById).openAPI(
            ...
            description:
                "Возвращает абонемент по UUID. Требует прав администратора.",
            response: .type(MembershipDTO.self),
            auth: .bearer()
        )

        routes.grouped(uuidMiddleware).get("user", ":user-id", use: getByUserId)
            .openAPI(
                tags: .init(name: "Admin - Membership"),
                summary:
                    "Получить абонементы по ID пользователя для администратора",
                description:
                    "Возвращает абонементы по UUID пользователя. Требует прав администратора.",
                response: .type([MembershipDTO].self),
                auth: .bearer()
            )
        ...
        routes.grouped(uuidMiddleware).get(
            "type",
            ":membership-type-id",
            use: getByMembershipTypeId
        ).openAPI(
            tags: .init(name: "Admin - Membership"),
            summary: "Получить абонементы по типу для администратора",
            description:
                "Возвращает абонементы по UUID типа абонемента. Требует прав администратора.",
            response: .type([MembershipDTO].self),
            auth: .bearer()
        )
    }
    ...
    fileprivate func registerDeleteRoutes(_ routes: RoutesBuilder) {
        routes.grouped(uuidMiddleware).delete(":id", use: deleteById).openAPI(
            ...
            description:
                "Удаляет абонемент по UUID. Требует прав администратора.",
...
extension MembershipAdminController {
    ...
    func getById(req: Request) async throws -> Response {
        ...
        return try await MembershipDTO(from: result)
            .encodeResponse(status: .ok, for: req)
    }
    ...
    func getByUserId(req: Request) async throws -> Response {
        try await service.find(
            userId: try req.parameters.require("user-id", as: UUID.self)
        ).map {
    ...
    func create(req: Request) async throws -> Response {
        guard
            let result = try await service.create(
                try req.content.decode(MembershipCreateDTO.self)
            )
