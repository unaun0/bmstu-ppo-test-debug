...
public final class AttendanceUserController: RouteCollection {
    ...
    public func boot(routes: RoutesBuilder) throws {
        ...
        attendanceRoutes.get(
            ...
            description:
                "Возвращает список всех записей на тренировки по абонементам пользователя.",
        ...
        attendanceRoutes.grouped(uuidMiddleware, dataMiddleware).post(
            ...
            description:
                "Пользователь записывается на тренировку с активным абонементом.",
        ...
        attendanceRoutes.grouped(uuidMiddleware).delete(
            ...
            description:
                "Отменяет запись на тренировку, если она ещё не состоялась.",
...
extension AttendanceUserController {
    ...
    func getMyAttendances(req: Request) async throws -> Response {
        ...
        let attendances = try await attendanceService.getMyAttendances(
            userId: userId
        )
        return try await attendances.map(AttendanceDTO.init(from:))
    ...
    func signUp(req: Request) async throws -> Response {
        ...
        guard let attendance else {
            throw Abort(
                .internalServerError,
                reason: "Не удалось создать запись на тренировку."
            )
        }
        return try await AttendanceDTO(from: attendance).encodeResponse(
            status: .ok,
            for: req
        )
    }
    ...
    func cancelAttendance(req: Request) async throws -> HTTPStatus {
        let userId = try req.auth.require(User.self).id
        let attendanceId = try req.parameters.require(
            "attendance-id",
            as: UUID.self
        )
        try await attendanceService.cancelAttendance(
