...
public struct MembershipValidationMiddleware: AsyncMiddleware {
    ...
    public func respond(
        ...
        do {
            if let membershipTypeId = try? request.content.get(
                String.self,
                at: "membershipTypeId"
            ) {
            ...
            let startDate = try? request.content.get(
                String.self,
                at: "startDate"
            ).toDate(
                format: ValidationRegex.DateFormat.format
            )
            let endDate = try? request.content.get(String.self, at: "endDate")
                .toDate(
                    format: ValidationRegex.DateFormat.format
            )
            ...
            if let sessions = try? request.content.get(
                Int.self,
                at: "availableSessions"
            ) {
                guard
